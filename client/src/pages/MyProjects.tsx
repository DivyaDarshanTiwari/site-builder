...
const MyProjects = () => {
    ...
    const deleteProject = async (projectId:string) => {
        try {
            const confirm = globalThis.confirm('Are you sure you want to delete this project?');
            if(!confirm) return;
      ...
      <div className='py-10 min-h-[80vh]' > 
          ...
          <div className='flex flex-wrap gap-3.5'>
              {projects.map((project)=>(
                  <div 
                      key={project.id} 
                      className='relative group w-72 max-sm:mx-auto bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden shadow-md group hover:shadow-indigo-700/30 hover:border-indigo-800/80 transition-all duration-300'>
                      ...
                      <div className='relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800'>
                          {project.current_code ? (
                              ...
                              title={`Project card preview ${project.id}`}
                       ...
                       <div className='p-4 text-white bg-linear-180 from-transparent group-hover:from-indigo-950 to-transparent transition-colors'>
                          <div className='flex items-start justify-between'>
                              ...
                              <button className='relative z-10 px-2.5 py-0.5 mt-1 ml-2 text-xs bg-gray-800 border border-gray-700 rounded-full'>Website</button>
                          </div>
                       ...
                       <TrashIcon className='absolute z-10 top-3 right-3 scale-0 group-hover:scale-100 bg-white p-1.5 size-7 rounded text-red-500 text-xl cursor-pointer transition-all' onClick={(e)=>{ e.preventDefault(); deleteProject(project.id) }}/>
                  </div>
              ))}
          </div>
