...
const Community = () => {
    ...
    const handleRemix = async (e: React.MouseEvent, projectId: string) => {
      ...
      e.stopPropagation();
      if (!session?.user) {
        toast.error("Please log in to remix a project");
        navigate('/auth/sign-in');
        return;
      }
      setRemixingId(projectId);
      try {
        const { data } = await api.post(`/api/project/remix/${projectId}`);
        toast.success("Project remixed! Opening your copy...");
                  ...
                  className='w-72 max-sm:mx-auto cursor-pointer bg-gray-900/60 border border-gray-700 rounded-lg overflow-hidden group hover:border-indigo-800/80 transition-all duration-300'>
                      ...
                      <div className='relative w-full h-40 bg-gray-900 overflow-hidden border-b border-gray-800'>
                          {project.current_code ? (
                              ...
                              title={`Community project preview ${project.id}`}
                          ...
                          <div className='flex justify-between items-center mt-6'>
                              ...
                              <div className='flex gap-2 text-white text-sm'>
                                  ...
                                  <button 
                                    ...
                                    className='px-3 py-1.5 bg-indigo-600/80 hover:bg-indigo-500 rounded-md transition-colors flex items-center gap-1.5 text-xs'>
