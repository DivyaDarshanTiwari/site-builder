...
export const exportProjectZip = async (req: Request, res: Response) => {
  try {
    ...
    // Sanitize the project name for use as a folder/file name
    ...
    // Add index.html
    archive.append(project.current_code, { name: `${safeName}/index.html` });
...
## How to Use
Open index.html in any web browser to view your website.
The site uses Tailwind CSS via CDN — an internet connection is needed for styling.
`;
    archive.append(readme, { name: `${safeName}/README.md` });

    await archive.finalize();
  } catch (error: any) {
    console.error("[exportProjectZip]", error);
    if (!res.headersSent) {
      res.status(500).json({ message: error.message });
    }
  }
